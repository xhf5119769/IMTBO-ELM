...
tr = templateTree('MaxNumSplits',4); % 定义单个决策树的模板
mdl = fitrensemble(fusion_train{i}', y_train{i}', 'Method', 'LSBoost', 'Learners', tr, 'NumLearningCycles', 10);
%% 进行预测

T{i} = predict(mdl, fusion_test{i}');
T{i}=T{i}';
    L=length(T{i});
    %% 指标计算
