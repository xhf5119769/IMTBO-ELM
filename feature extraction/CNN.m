...
for i=1:10
  ...
  [net, info] = trainNetwork(X_train{i}, X_train{i}, layers_19, options);
  layerName = 'conv1d_2_2';
  Xn_train{i}= activations(net, X_train{i},  layerName, 'OutputAs', 'channels');
  Xn_test{i}= activations(net, X_test{i},  layerName, 'OutputAs', 'channels');
  N=200;%%隐含层节点
  [IW,B,LW,TF,TYPE] = elmtrain(Xn_train{1,i}{1,1},y_train{i},N,'sig');
  T{i}= elmpredict(Xn_test{1,i}{1,1},IW,B,LW,TF,TYPE);
    L=length(T{i});
    %% 指标计算
